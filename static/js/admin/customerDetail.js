...
function initData() {
    ...
    if (id) {
        asyncInvoke("/customer.api", "GetItem", { id: id }, function(d) {
            if (d.code) {
                errHandler(d.data);
                return
            }
...
function getDataFromDom() {
    ...
    if (data.createTime) {
        ...
        data.createTime = new Date("1900-01-01");
    }
    if (data.updateTime) {
        data.updateTime = new Date(data.updateTime);
    } else {
        data.updateTime = new Date("1900-01-01");
    }
...
function save() {
    ...
    asyncInvoke("/customer.api", method, jsonData, function(d) {
        if (d.code === 0) {
            alert("保存成功")
            window.location.href = "/admin/customerDetail?id=" + d.data
        } else {
            errHandler(d.data)
        }
    })
